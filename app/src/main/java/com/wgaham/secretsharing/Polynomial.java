...
public class Polynomial {
    ...
    public int[] coef; //coefficients 系数
    public int deg;//指数，次数 degree of polynomial (0 for the zero polynomial)
    ...
    // 求导
    public Polynomial differentiate(int d) {
    ...
    public int polynomial1D1(Polynomial polynomial, int x) {
        int result = 0;
        polynomial.coef = coef;
        for (int i = coef.length - 1; i >= 0; i--) {
            result = result * x + coef[i];
    ...
    public int cal(Polynomial polynomial, int b, int x, int e) {
        int result = 0;
        //假设存在一个初始多项式系数的数组长度l
